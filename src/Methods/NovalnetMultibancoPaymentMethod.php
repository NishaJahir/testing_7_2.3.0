 ...
 */
class NovalnetMultibancoPaymentMethod extends PaymentMethodService
{
    /**
    ...
    {
       if ($this->configRepository->get('Novalnet.novalnet_multibanco_payment_active') == 'true') {
        
        $active_payment_allowed_country = true;
        if ($allowed_country = $this->configRepository->get('Novalnet.novalnet_multibanco_allowed_country')) {
        ...
        $minimum_amount = trim($this->configRepository->get('Novalnet.novalnet_multibanco_minimum_order_amount'));
        ...
        $maximum_amount = trim($this->configRepository->get('Novalnet.novalnet_multibanco_maximum_order_amount'));
    ...
    {  
        return $this->paymentHelper->getCustomizedTranslatedText('paymentmethod_novalnet_multibanco');
    }
    ...
    {
       $logoUrl = $this->configRepository->get('Novalnet.novalnet_multibanco_payment_logo');
        if($logoUrl == 'images/multibanco.png'){
            /** @var Application $app */
            $app = pluginApp(Application::class);
            $logoUrl = $app->getUrlPath('novalnet') .'/images/multibanco.png';
        } 
        return $logoUrl;
    }
    ...
    {
        return $this->paymentHelper->getCustomizedTranslatedText('paymentmethod_novalnet_multibanco_payment_description');
    }
