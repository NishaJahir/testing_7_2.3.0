 ...
 */
class NovalnetPostfinanceCardPaymentMethod extends PaymentMethodService
{
    /**
    ...
    {
       if ($this->configRepository->get('Novalnet.novalnet_postfinance_card_payment_active') == 'true') {
        
        $active_payment_allowed_country = true;
        if ($allowed_country = $this->configRepository->get('Novalnet.novalnet_postfinance_card_allowed_country')) {
        ...
        $minimum_amount = trim($this->configRepository->get('Novalnet.novalnet_postfinance_card_minimum_order_amount'));
        ...
        $maximum_amount = trim($this->configRepository->get('Novalnet.novalnet_postfinance_card_maximum_order_amount'));
    ...
    {   
        return $this->paymentHelper->getCustomizedTranslatedText('paymentmethod_novalnet_postfinance_card');
    }
    ...
    {
        $logoUrl = $this->configRepository->get('Novalnet.novalnet_postfinance_card_payment_logo');
        if($logoUrl == 'images/postfinance_card.png'){
            /** @var Application $app */
            $app = pluginApp(Application::class);
            $logoUrl = $app->getUrlPath('novalnet') .'/images/postfinance_card.png';
        } 
        return $logoUrl;
    }
    ...
    {
       return $this->paymentHelper->getCustomizedTranslatedText('paymentmethod_novalnet_postfinance_card_payment_description');
    }
