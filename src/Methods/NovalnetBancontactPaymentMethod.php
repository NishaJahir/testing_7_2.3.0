...
class NovalnetBancontactPaymentMethod extends PaymentMethodService
{
    ...
    public function isActive():bool
    {
       if ($this->configRepository->get('Novalnet.novalnet_bancontact_payment_active') == 'true') {
        ...
        if ($allowed_country = $this->configRepository->get('Novalnet.novalnet_bancontact_allowed_country')) {
        ...
        $minimum_amount = trim($this->configRepository->get('Novalnet.novalnet_bancontact_minimum_order_amount'));
        ...
        $maximum_amount = trim($this->configRepository->get('Novalnet.novalnet_bancontact_maximum_order_amount'));
    ...
    {   
        return $this->paymentHelper->getCustomizedTranslatedText('paymentmethod_novalnet_bancontact');
    }
    ...
    {
        $logoUrl = $this->configRepository->get('Novalnet.novalnet_bancontact_payment_logo');
        if($logoUrl == 'images/bancontact.png'){
            /** @var Application $app */
            $app = pluginApp(Application::class);
            $logoUrl = $app->getUrlPath('novalnet') .'/images/bancontact.png';
        } 
        return $logoUrl;
    }
    ...
    {
       return $this->paymentHelper->getCustomizedTranslatedText('paymentmethod_novalnet_bancontact_payment_description');
    }
