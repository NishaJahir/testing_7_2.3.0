 ...
 */
class NovalnetApplePayPaymentMethod extends PaymentMethodService
{
    ...
    {
       if ($this->configRepository->get('Novalnet.novalnet_applepay_payment_active') == 'true') {
        
        $active_payment_allowed_country = true;
        if ($allowed_country = $this->configRepository->get('Novalnet.novalnet_applepay_allowed_country')) {
        ...
        $minimum_amount = trim($this->configRepository->get('Novalnet.novalnet_applepay_minimum_order_amount'));
        ...
        $maximum_amount = trim($this->configRepository->get('Novalnet.novalnet_applepay_maximum_order_amount'));
    ...
    {   
        return $this->paymentHelper->getCustomizedTranslatedText('paymentmethod_novalnet_applepay');
    }
    ...
    {
        $logoUrl = $this->configRepository->get('Novalnet.novalnet_applepay_payment_logo');
        if($logoUrl == 'images/applepay.png'){
            /** @var Application $app */
            $app = pluginApp(Application::class);
            $logoUrl = $app->getUrlPath('novalnet') .'/images/applepay.png';
        } 
        return $logoUrl;
    ...
    {
         return $this->paymentHelper->getCustomizedTranslatedText('paymentmethod_novalnet_applepay_payment_description');
    }
